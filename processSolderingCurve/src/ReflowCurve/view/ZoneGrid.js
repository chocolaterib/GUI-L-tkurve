  ...
  group.selectAll('rect.zone-hitbox')
    ...
    .join(
      enter => enter.append('rect')
        .attr('class', 'zone-hitbox')
        .style('fill', 'transparent')
        .style('cursor', 'pointer')
        .style('pointer-events', 'all'), // Muss events fangen
      update => update,
      exit => exit.remove()
    )
    .attr('x', d => Math.min(scales.x(d.xStart), scales.x(d.xEnd)))
    ...
    .on('click', (event, d) => {
        ...
        if (onZoneClick) onZoneClick(d);
    })
    .on('mouseenter', function(event, d) {
        // 1. Timer löschen (falls wir schnell von Zone zu Zone wechseln)
        if (tooltipHideTimer) {
            clearTimeout(tooltipHideTimer);
            tooltipHideTimer = null;
        }
        ...
        if (renderTooltip) {
             const htmlContent = renderTooltip(d);
             tooltip.html(htmlContent);
             tooltip.style('pointer-events', 'auto'); // Interaktion erlauben

             // Wir suchen jetzt nach dem Container-Div aus dem Template
             // Das Template muss <div class="tooltip-chart-window"><div class="tooltip-chart-content"></div></div> enthalten
             // Oder wir bauen es hier dynamisch, falls das Template einfach ist.
             // Annahme: Dein Template hat ein leeres <div class="tooltip-chart"></div>
             const chartContainer = tooltip.select('.tooltip-chart');
             
             if (!chartContainer.empty()) {
                 // Wir bauen die Struktur für Scrollbarkeit
                 // 1. Klasse ändern für CSS Styling
                 chartContainer.attr('class', 'tooltip-chart-window');
                 
                 // 2. Inneren Container erzeugen (falls noch nicht da)
                 let innerContent = chartContainer.select('.tooltip-chart-content');
                 if (innerContent.empty()) {
                     innerContent = chartContainer.append('div').attr('class', 'tooltip-chart-content');
                 }

                 drawScrollableChart({
                    windowDiv: chartContainer,
                    contentDiv: innerContent,
                    series, istGraph, totalTime, startTime, maxTemperature,
                    zoneStart: d.xStart,
                    zoneEnd: d.xEnd,
                    globalStartVal: startVal
                 });
             }
    ...
    .on('mousemove', function(event) {
        // Tooltip nur bewegen, wenn er NICHT gerade interagiert wird?
        // Bei "interaktiven Tooltips" ist es oft besser, den Tooltip NICHT stur an die Maus zu kleben,
        // sobald er offen ist, damit er nicht "unter der Maus wegrennt", wenn man hin will.
        // Wir fixieren ihn leicht versetzt.
        
        // Position nur updaten, wenn Maus weit genug weg ist, um Flackern zu vermeiden
        // Oder simpler: Einfach mitbewegen.
        
        // ACHTUNG: Wenn wir wollen, dass man in den Tooltip gehen kann, muss er nah genug sein!
        // Wir setzen ihn z.B. 10px unter/neben die Maus.
        tooltip
    ...
    .on('mouseleave', function() {
        ...
        // Verzögerung starten! Das gibt dem Nutzer Zeit, die Maus in den Tooltip zu bewegen.
        tooltipHideTimer = setTimeout(() => {
            tooltip.style('opacity', 0).style('pointer-events', 'none');
        }, 300); // 300ms Zeit um rüber zu gehen
    });
...
function drawScrollableChart({ 
  ...
  zoneStart, zoneEnd, globalStartVal 
    ...
    // 5. Kurven zeichnen
    renderLineSeries(miniState, shell.ctx);
